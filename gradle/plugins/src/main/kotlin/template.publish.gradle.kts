 ...
 * (CLAUDE.md §8).
 *
 * All modules publish together — same group, same version (inherited from
 ...
 * skipped — fine for `publishToMavenLocal` dry-runs (mise: publish:local).
 *
 * The group prefix `com.happycodelucky` and org `happycodelucky` are the
 * at-rest defaults; `mise run init <name> --group … --org …` rewrites them.
 */
