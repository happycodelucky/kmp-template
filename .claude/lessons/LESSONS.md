...
- **N-008** — Kotlin 2.4 removed `abiValidation.enabled`: presence of the block enables validation, so a module can only be *exempted* by the convention plugin skipping the block for it (`enabled.set(false)` has no replacement). Prove the result from the task graph — silently dropping validation from a published module looks exactly like success.
- **N-009** — A green Gradle run proves nothing if it came from cache. When a verification finishes suspiciously fast, re-run with `--rerun-tasks --no-build-cache` and check the executed-task count.
- **D-003** — Port findings between repos, not config. Tuning derived from one repo's observed output is cargo-culting elsewhere, and lesson-file ID schemes collide across repos (`N-` means "Notes" here, "NEVER DO" in `reachable`).
